...
The midband gain is obtained from the flat region of the Bode plot.  
The bandwidth is defined as the frequency range between the lower cutoff frequency (fL) and upper cutoff frequency (fH), measured at the −3 dB points.
...
Since the 0 dB crossing is not visible in the plot, UGB is approximated as:
...
| Parameter | Theoretical | Simulated |
|----------|------------|----------|
| Voltage Gain (V/V) | ~30 V/V | ~41.6 V/V |
| Gain (dB) | ~29.5 dB | 32.37 dB |

Deviation occurs due to:

- PMOS active load behaving as a current mirror  
- Increased effective output resistance  
- Channel length modulation  
- Bias-dependent variations  


## Inference

The MOS differential amplifier with PMOS active load and NMOS current source was designed under the following constraints:
...
The biasing ensures all transistors (M1, M2, M3, M4, M5) operate in saturation.

The tail node voltage is approximately:
...
- Flat midband gain region observed  
- Gain roll-off at high frequency due to parasitic capacitances  
- Dominant pole formed due to load capacitance (CL)  
- Bandwidth limited by intrinsic device capacitances  
...
- Current shared between both branches  
...
|----------|----------|----------|----------|
| Load Type | Resistive | Resistive | PMOS Active Load |
| Tail Element | Ideal Source | NMOS Source | NMOS Source |
| Supply Voltage | ±0.9V | ±0.9V | ±0.9V |
| Tail Current | 1.22 mA | 1.22 mA | 1.22 mA |
| Current per Branch | 0.61 mA | 0.61 mA | 0.61 mA |
| Gain (V/V) | Moderate | Low | High |
| Gain (dB) | ~15–20 dB | Low | 32.37 dB |
...
Three differential amplifier configurations were analyzed under:
...
- Resistive loads → stable, moderate gain  
- NMOS current source → reduces gain due to degeneration  
- PMOS active load → highest gain due to current mirror action  

Thus, load type and biasing significantly affect gain, bandwidth, and linearity in CMOS differential amplifiers.
